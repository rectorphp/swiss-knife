 ...
 */
namespace SwissKnife202606\Symfony\Component\Yaml;

use SwissKnife202606\Symfony\Component\Yaml\Exception\DumpException;
use SwissKnife202606\Symfony\Component\Yaml\Exception\ParseException;
use SwissKnife202606\Symfony\Component\Yaml\Tag\TaggedValue;
/**
                ...
                switch (\true) {
                    ...
                    case \strncmp($scalar, '!php/object', \strlen('!php/object')) === 0:
                        if (self::$objectSupport) {
                            ...
                            $serialized = self::parseScalar((string) \substr($scalar, 12));
                            if (!\is_scalar($serialized ?? '') && !(\is_object($serialized) && \method_exists($serialized, '__toString'))) {
                    ...
                    case \strncmp($scalar, '!php/const', \strlen('!php/const')) === 0:
                        if (self::$constantSupport) {
                            ...
                            $i = 0;
                            $const = self::parseScalar((string) \substr($scalar, 11), 0, null, $i, \false);
                            if (!\is_scalar($const ?? '') && !(\is_object($const) && \method_exists($const, '__toString'))) {
                    ...
                    case \strncmp($scalar, '!php/enum', \strlen('!php/enum')) === 0:
                        if (self::$constantSupport) {
                            ...
                            if (!\is_scalar($enumName ?? '') && !(\is_object($enumName) && \method_exists($enumName, '__toString'))) {
