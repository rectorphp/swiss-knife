...
class Interval
{
    ...
    /** @var Constraint */
    private $end;
    public function __construct(\EasyCI20220115\Composer\Semver\Constraint\Constraint $start, \EasyCI20220115\Composer\Semver\Constraint\Constraint $end)
    {
        $this->start = $start;
        $this->end = $end;
    }
    ...
    public static function fromZero()
    {
        static $zero;
        if (null === $zero) {
            $zero = new \EasyCI20220115\Composer\Semver\Constraint\Constraint('>=', '0.0.0.0-dev');
        }
        return $zero;
    }
    /**
    ...
    public static function untilPositiveInfinity()
    {
        ...
        if (null === $positiveInfinity) {
            $positiveInfinity = new \EasyCI20220115\Composer\Semver\Constraint\Constraint('<', \PHP_INT_MAX . '.0.0.0');
        }
