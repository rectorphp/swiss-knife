...
namespace EasyCI20220414\Nette\Loaders;
...
class RobotLoader
{
    ...
    public function __construct()
    {
        if (!\extension_loaded('tokenizer')) {
            throw new \EasyCI20220414\Nette\NotSupportedException('PHP extension Tokenizer is not loaded.');
        }
    }
            ...
            foreach ($iterator as $fileInfo) {
                ...
                foreach ($foundClasses as $class) {
                    if (isset($this->classes[$class])) {
                        throw new \EasyCI20220414\Nette\InvalidStateException("Ambiguous class {$class} resolution; defined in {$this->classes[$class][0]} and in {$file}.");
                    }
     ...
     */
    private function createFileIterator(string $dir) : \EasyCI20220414\Nette\Utils\Finder
    {
        if (!\is_dir($dir)) {
            throw new \EasyCI20220414\Nette\IOException("File or directory '{$dir}' not found.");
        }
        ...
        $iterator = \EasyCI20220414\Nette\Utils\Finder::findFiles($acceptFiles)->filter(function (\SplFileInfo $file) use(&$disallow) {
    ...
    private function updateFile(string $file) : void
    {
        ...
        foreach ($foundClasses as $class) {
            ...
            if (isset($prevFile)) {
                throw new \EasyCI20220414\Nette\InvalidStateException("Ambiguous class {$class} resolution; defined in {$prevFile} and in {$file}.");
            }
    ...
    {
        \EasyCI20220414\Nette\Utils\FileSystem::createDir($dir);
