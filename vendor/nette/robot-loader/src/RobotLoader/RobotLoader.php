...
namespace SwissKnife202412\Nette\Loaders;

use SwissKnife202412\Nette;
...
class RobotLoader
{
    ...
    private bool $autoRebuild = \true;
    private bool $reportParseErrors = \true;
    /** @var string[] */
    ...
    private function createFileIterator(string $dir) : Nette\Utils\Finder
    {
        ...
        return Nette\Utils\Finder::findFiles($this->acceptFiles)->filter($filter = fn(SplFileInfo $file) => $file->getRealPath() === \false || !isset($disallow[$file->getRealPath()]))->descentFilter($filter)->from($dir)->exclude($this->ignoreDirs);
    }
