...
final class Traverser
{
     ...
     */
    public function traverse(\EasyCI20220313\Nette\Neon\Node $node, ?callable $enter = null, ?callable $leave = null) : \EasyCI20220313\Nette\Neon\Node
    {
        $this->enter = $enter;
        $this->leave = $leave;
        $this->stop = \false;
    ...
    private function traverseNode(\EasyCI20220313\Nette\Neon\Node $node) : \EasyCI20220313\Nette\Neon\Node
    {
        $children = \true;
        if ($this->enter) {
            $res = ($this->enter)($node);
            if ($res instanceof \EasyCI20220313\Nette\Neon\Node) {
                $node = $res;
