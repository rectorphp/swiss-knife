...
namespace EasyCI20220530\Symplify\Astral\Naming;

use EasyCI20220530\Nette\Utils\Strings;
use EasyCI20220530\PhpParser\Node;
use EasyCI20220530\PhpParser\Node\Expr\ClassConstFetch;
...
final class SimpleNameResolver
{
    ...
    public function getName($node) : ?string
    {
        ...
        if ($node instanceof \EasyCI20220530\PhpParser\Node\Expr\ClassConstFetch && $this->isName($node->name, 'class')) {
            return $this->getName($node->class);
        }
        if ($node instanceof \EasyCI20220530\PhpParser\Node\Stmt\Property) {
            $propertyProperty = $node->props[0];
            return $this->getName($propertyProperty->name);
        }
        if ($node instanceof \EasyCI20220530\PhpParser\Node\Expr\Variable) {
     ...
     */
    public function isNames(\EasyCI20220530\PhpParser\Node $node, array $desiredNames) : bool
    {
    ...
    public function areNamesEqual(\EasyCI20220530\PhpParser\Node $firstNode, \EasyCI20220530\PhpParser\Node $secondNode) : bool
    {
    ...
    public function resolveShortNameFromNode(\EasyCI20220530\PhpParser\Node\Stmt\ClassLike $classLike) : ?string
    {
        ...
        if (\EasyCI20220530\Nette\Utils\Strings::match($className, self::ANONYMOUS_CLASS_REGEX)) {
            return null;
        }
        return $this->resolveShortName($className);
    }
    public function resolveShortNameFromScope(\EasyCI20220530\PHPStan\Analyser\Scope $scope) : ?string
    {
    ...
    public function getClassNameFromScope(\EasyCI20220530\PHPStan\Analyser\Scope $scope) : ?string
    {
        if ($scope->isInTrait()) {
            $traitReflection = $scope->getTraitReflection();
            if (!$traitReflection instanceof \EasyCI20220530\PHPStan\Reflection\ClassReflection) {
                return null;
            }
    ...
    public function isNameMatch(\EasyCI20220530\PhpParser\Node $node, string $desiredNameRegex) : bool
    {
        $name = $this->getName($node);
        if ($name === null) {
            return \false;
        }
        return (bool) \EasyCI20220530\Nette\Utils\Strings::match($name, $desiredNameRegex);
    }
    public function resolveShortName(string $className) : string
    {
        ...
        return (string) \EasyCI20220530\Nette\Utils\Strings::after($className, '\\', -1);
    }
}
