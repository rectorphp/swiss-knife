...
use EasyCI20220607\Symplify\Astral\PhpDocParser\Contract\PhpDocNodeVisitorInterface;
use EasyCI20220607\Symplify\Astral\PhpDocParser\Exception\InvalidTraverseException;
use EasyCI20220607\Symplify\Astral\PhpDocParser\PhpDocNodeVisitor\CallablePhpDocNodeVisitor;
/**
 * @api
 *
 * Mimics
...
final class PhpDocNodeTraverser
{
    ...
    public function addPhpDocNodeVisitor(\EasyCI20220607\Symplify\Astral\PhpDocParser\Contract\PhpDocNodeVisitorInterface $phpDocNodeVisitor) : void
    {
        $this->phpDocNodeVisitors[] = $phpDocNodeVisitor;
    }
    public function traverse(\EasyCI20220607\PHPStan\PhpDocParser\Ast\Node $node) : void
    {
     ...
     */
    public function traverseWithCallable(\EasyCI20220607\PHPStan\PhpDocParser\Ast\Node $node, string $docContent, callable $callable) : \EasyCI20220607\PHPStan\PhpDocParser\Ast\Node
    {
        $callablePhpDocNodeVisitor = new \EasyCI20220607\Symplify\Astral\PhpDocParser\PhpDocNodeVisitor\CallablePhpDocNodeVisitor($callable, $docContent);
     ...
     * @return TNode
     */
    private function traverseNode(\EasyCI20220607\PHPStan\PhpDocParser\Ast\Node $node) : \EasyCI20220607\PHPStan\PhpDocParser\Ast\Node
    {
        ...
        foreach ($subNodeNames as $subNodeName) {
            ...
            if (\is_array($subNode)) {
            ...
            } elseif ($subNode instanceof \EasyCI20220607\PHPStan\PhpDocParser\Ast\Node) {
                ...
                foreach ($this->phpDocNodeVisitors as $visitorIndex => $phpDocNodeVisitor) {
                    ...
                    if ($return !== null) {
                        if ($return instanceof \EasyCI20220607\PHPStan\PhpDocParser\Ast\Node) {
                            $subNode = $return;
                            ...
                            throw new \EasyCI20220607\Symplify\Astral\PhpDocParser\Exception\InvalidTraverseException('enterNode() returned invalid value of type ' . \gettype($return));
                        }
                    }
                }
                if ($traverseChildren) {
    ...
    private function traverseArray(array $nodes) : array
    {
        foreach ($nodes as $key => &$node) {
            ...
            if (!$node instanceof \EasyCI20220607\PHPStan\PhpDocParser\Ast\Node) {
                continue;
            }
            $traverseChildren = \true;
            $breakVisitorIndex = null;
            ...
            foreach ($this->phpDocNodeVisitors as $visitorIndex => $phpDocNodeVisitor) {
                ...
                if ($return !== null) {
                    if ($return instanceof \EasyCI20220607\PHPStan\PhpDocParser\Ast\Node) {
                        ...
                        throw new \EasyCI20220607\Symplify\Astral\PhpDocParser\Exception\InvalidTraverseException('leaveNode() returned invalid value of type ' . \gettype($return));
                    }
                }
                if ($breakVisitorIndex === $visitorIndex) {
                    break;
