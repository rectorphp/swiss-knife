...
namespace EasyCI20220325\Symplify\PackageBuilder\Testing;

use EasyCI20220325\PHPUnit\Framework\TestCase;
use ReflectionClass;
...
use EasyCI20220325\Symfony\Component\HttpKernel\KernelInterface;
use EasyCI20220325\Symfony\Contracts\Service\ResetInterface;
use EasyCI20220325\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface;
use EasyCI20220325\Symplify\PackageBuilder\Exception\HttpKernel\MissingInterfaceException;
use EasyCI20220325\Symplify\SmartFileSystem\SmartFileInfo;
use EasyCI20220325\Symplify\SymplifyKernel\Contract\LightKernelInterface;
 ...
 */
abstract class AbstractKernelTestCase extends \EasyCI20220325\PHPUnit\Framework\TestCase
{
    /**
    ...
    protected function getService(string $type)
    {
        if (self::$container === null) {
            throw new \EasyCI20220325\Symplify\SymplifyKernel\Exception\ShouldNotHappenException('First, create container with booKernel(KernelClass::class)');
        }
        $service = self::$container->get($type);
        if ($service === null) {
            $errorMessage = \sprintf('Services "%s" was not found', $type);
            throw new \EasyCI20220325\Symplify\Astral\Exception\ShouldNotHappenException($errorMessage);
        }
        return $service;
    }
    /**
    ...
    {
        if (\is_a($kernelClass, \EasyCI20220325\Symplify\SymplifyKernel\Contract\LightKernelInterface::class, \true)) {
        ...
        if (!$kernel instanceof \EasyCI20220325\Symfony\Component\HttpKernel\KernelInterface) {
    ...
    {
        if (static::$kernel !== null && static::$kernel instanceof \EasyCI20220325\Symfony\Component\HttpKernel\KernelInterface) {
            ...
            if ($kernel !== null) {
                ...
                if ($container instanceof \EasyCI20220325\Symfony\Contracts\Service\ResetInterface) {
                    $container->reset();
                }
            }
        }
    ...
    protected function resolveConfigFilePaths(array $configs) : array
    {
        $configFilePaths = [];
        foreach ($configs as $config) {
            $configFilePaths[] = $config instanceof \EasyCI20220325\Symplify\SmartFileSystem\SmartFileInfo ? $config->getRealPath() : $config;
        }
    ...
    {
        if ($kernel instanceof \EasyCI20220325\Symplify\SymplifyKernel\Contract\LightKernelInterface) {
            return;
        }
        if ($kernel instanceof \EasyCI20220325\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface) {
            return;
        }
        throw new \EasyCI20220325\Symplify\PackageBuilder\Exception\HttpKernel\MissingInterfaceException(\sprintf('"%s" is missing an "%s" interface', \get_class($kernel), \EasyCI20220325\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface::class));
    }
    private function bootAndReturnKernel(\EasyCI20220325\Symfony\Component\HttpKernel\KernelInterface $kernel) : \EasyCI20220325\Symfony\Component\HttpKernel\KernelInterface
    {
        $kernel->boot();
        ...
        if (!$container instanceof \EasyCI20220325\Symfony\Component\DependencyInjection\ContainerInterface) {
            throw new \EasyCI20220325\Symplify\SymplifyKernel\Exception\ShouldNotHappenException();
        }
        // has output? keep it silent out of tests
        if ($container->has(\EasyCI20220325\Symfony\Component\Console\Style\SymfonyStyle::class)) {
            $symfonyStyle = $container->get(\EasyCI20220325\Symfony\Component\Console\Style\SymfonyStyle::class);
            $symfonyStyle->setVerbosity(\EasyCI20220325\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_QUIET);
        }
