...
namespace EasyCI20220527\Symplify\VendorPatches\Differ;
...
final class PatchDiffer
{
     ...
     */
    private $differ;
    public function __construct(\EasyCI20220527\SebastianBergmann\Diff\Differ $differ)
    {
        $this->differ = $differ;
    }
    public function diff(\EasyCI20220527\Symplify\VendorPatches\ValueObject\OldAndNewFileInfo $oldAndNewFileInfo) : string
    {
        ...
        $clearedDiff = \EasyCI20220527\Nette\Utils\Strings::replace($diff, self::START_ORIGINAL_REGEX, '--- /dev/null');
        return \EasyCI20220527\Nette\Utils\Strings::replace($clearedDiff, self::START_NEW_REGEX, '+++ ' . $patchedFileRelativePath);
    }
    private function resolveFileInfoPathRelativeFilePath(\EasyCI20220527\Symplify\SmartFileSystem\SmartFileInfo $beforeFileInfo) : string
    {
        $match = \EasyCI20220527\Nette\Utils\Strings::match($beforeFileInfo->getRealPath(), self::LOCAL_PATH_REGEX);
