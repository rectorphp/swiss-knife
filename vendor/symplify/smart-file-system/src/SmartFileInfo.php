...
namespace EasyCI20220522\Symplify\SmartFileSystem;

use EasyCI20220522\Nette\Utils\Strings;
use EasyCI20220522\Symfony\Component\Finder\SplFileInfo;
use EasyCI20220522\Symplify\EasyTesting\PHPUnit\StaticPHPUnitEnvironment;
use EasyCI20220522\Symplify\EasyTesting\StaticFixtureSplitter;
use EasyCI20220522\Symplify\SmartFileSystem\Exception\DirectoryNotFoundException;
use EasyCI20220522\Symplify\SmartFileSystem\Exception\FileNotFoundException;
/**
 * @see \Symplify\SmartFileSystem\Tests\SmartFileInfo\SmartFileInfoTest
 */
final class SmartFileInfo extends \EasyCI20220522\Symfony\Component\Finder\SplFileInfo
{
    /**
     * @var string
    ...
    {
        $this->smartFileSystem = new \EasyCI20220522\Symplify\SmartFileSystem\SmartFileSystem();
        // accepts also dirs
        if (!\file_exists($filePath)) {
            throw new \EasyCI20220522\Symplify\SmartFileSystem\Exception\FileNotFoundException(\sprintf('File path "%s" was not found while creating "%s" object.', $filePath, self::class));
        }
    ...
    {
        return \EasyCI20220522\Nette\Utils\Strings::replace($this->getRealPath(), self::LAST_SUFFIX_REGEX, '');
    }
    ...
    public function getRelativeFilePathFromDirectory(string $directory) : string
    {
        if (!\file_exists($directory)) {
            throw new \EasyCI20220522\Symplify\SmartFileSystem\Exception\DirectoryNotFoundException(\sprintf('Directory "%s" was not found in %s.', $directory, self::class));
        }
    ...
    public function getRelativeFilePathFromCwdInTests() : string
    {
        ...
        if (\EasyCI20220522\Symplify\EasyTesting\PHPUnit\StaticPHPUnitEnvironment::isPHPUnitRun()) {
            return $this->getRelativeFilePathFromDirectory(\EasyCI20220522\Symplify\EasyTesting\StaticFixtureSplitter::getTemporaryPath());
        }
