...
{
    return \preg_replace_callback('~\\b(?<name>[A-Z][a-zA-Z_\\\\]++)\\s*' . \EasyCI202207\PARAMS . '~', function ($matches) {
        // recurse
        $matches['params'] = resolveNodes($matches['params']);
        $params = magicSplit('(?:' . \EasyCI202207\PARAMS . '|' . \EasyCI202207\ARGS . ')(*SKIP)(*FAIL)|,', $matches['params']);
        $paramCode = '';
...
{
    return \preg_replace_callback('~\\b(?<!::|->)(?!array\\()(?<name>[a-z][A-Za-z]++)' . \EasyCI202207\ARGS . '~', function ($matches) {
        // recurse
        $matches['args'] = resolveMacros($matches['args']);
        $name = $matches['name'];
        $args = magicSplit('(?:' . \EasyCI202207\PARAMS . '|' . \EasyCI202207\ARGS . ')(*SKIP)(*FAIL)|,', $matches['args']);
        if ('attributes' === $name) {
...
{
    return '~' . \EasyCI202207\LIB . '(?:' . \str_replace('~', '\\~', $regex) . ')~';
}
