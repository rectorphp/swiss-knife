...
{
    return \preg_replace_callback('~\\b(?<name>[A-Z][a-zA-Z_\\\\]++)\\s*' . \EasyCI20220608\PARAMS . '~', function ($matches) {
        // recurse
        $matches['params'] = resolveNodes($matches['params']);
        $params = magicSplit('(?:' . \EasyCI20220608\PARAMS . '|' . \EasyCI20220608\ARGS . ')(*SKIP)(*FAIL)|,', $matches['params']);
        $paramCode = '';
...
{
    return \preg_replace_callback('~\\b(?<!::|->)(?!array\\()(?<name>[a-z][A-Za-z]++)' . \EasyCI20220608\ARGS . '~', function ($matches) {
        // recurse
        $matches['args'] = resolveMacros($matches['args']);
        $name = $matches['name'];
        $args = magicSplit('(?:' . \EasyCI20220608\PARAMS . '|' . \EasyCI20220608\ARGS . ')(*SKIP)(*FAIL)|,', $matches['args']);
        if ('attributes' === $name) {
...
{
    return '~' . \EasyCI20220608\LIB . '(?:' . \str_replace('~', '\\~', $regex) . ')~';
}
