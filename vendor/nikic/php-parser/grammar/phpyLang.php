...
{
    $code = \EasyCI20220602\resolveNodes($code);
    $code = \EasyCI20220602\resolveMacros($code);
    $code = \EasyCI20220602\resolveStackAccess($code);
    return $code;
}
function resolveNodes($code)
{
    return \preg_replace_callback('~\\b(?<name>[A-Z][a-zA-Z_\\\\]++)\\s*' . \PARAMS . '~', function ($matches) {
        // recurse
        $matches['params'] = \EasyCI20220602\resolveNodes($matches['params']);
        $params = \EasyCI20220602\magicSplit('(?:' . \PARAMS . '|' . \ARGS . ')(*SKIP)(*FAIL)|,', $matches['params']);
        $paramCode = '';
...
function resolveMacros($code)
{
    return \preg_replace_callback('~\\b(?<!::|->)(?!array\\()(?<name>[a-z][A-Za-z]++)' . \ARGS . '~', function ($matches) {
        // recurse
        $matches['args'] = \EasyCI20220602\resolveMacros($matches['args']);
        $name = $matches['name'];
        $args = \EasyCI20220602\magicSplit('(?:' . \PARAMS . '|' . \ARGS . ')(*SKIP)(*FAIL)|,', $matches['args']);
        if ('attributes' === $name) {
            \EasyCI20220602\assertArgs(0, $args, $name);
            return '$this->startAttributeStack[#1] + $this->endAttributes';
        }
        if ('stackAttributes' === $name) {
            \EasyCI20220602\assertArgs(1, $args, $name);
        ...
        if ('push' === $name) {
            \EasyCI20220602\assertArgs(2, $args, $name);
        ...
        if ('parseEncapsed' === $name) {
            \EasyCI20220602\assertArgs(3, $args, $name);
...
{
    $pieces = \preg_split(\EasyCI20220602\regex('(?:(?&string)|(?&comment)|(?&code))(*SKIP)(*FAIL)|' . $regex), $string);
