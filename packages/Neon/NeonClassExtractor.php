...
use EasyCI20220418\Nette\Neon\Node\LiteralNode;
...
final class NeonClassExtractor
{
     ...
     * @return string[]
     */
    public function extract(\EasyCI20220418\Symplify\SmartFileSystem\SmartFileInfo $fileInfo) : array
    {
        $neonDecoder = new \EasyCI20220418\Nette\Neon\Decoder();
    ...
    private function hasKeyValue(\EasyCI20220418\Nette\Neon\Node\ArrayItemNode $arrayItemNode, string $value) : bool
    {
        if (!$arrayItemNode->key instanceof \EasyCI20220418\Nette\Neon\Node\LiteralNode) {
            return \false;
        }
     ...
     * @return string[]
     */
    private function findClassNames(\EasyCI20220418\Nette\Neon\Node $node) : array
    {
        $classNames = [];
        $traverser = new \EasyCI20220418\Nette\Neon\Traverser();
        $traverser->traverse($node, function (\EasyCI20220418\Nette\Neon\Node $node) use(&$classNames) : ?Node {
            ...
            if ($node->value instanceof \EasyCI20220418\Nette\Neon\Node\LiteralNode) {
     ...
     * @return string[]
     */
    private function findsStringStaticCallReferences(\EasyCI20220418\Nette\Neon\Node $node) : array
    {
        $classNames = [];
        $traverser = new \EasyCI20220418\Nette\Neon\Traverser();
        $traverser->traverse($node, function (\EasyCI20220418\Nette\Neon\Node $node) use(&$classNames) {
            if (!$node instanceof \EasyCI20220418\Nette\Neon\Node\LiteralNode) {
                return null;
            }
     ...
     * @return string[]
     */
    private function findsServicesKeyList(\EasyCI20220418\Nette\Neon\Node $node) : array
    {
        $classNames = [];
        $traverser = new \EasyCI20220418\Nette\Neon\Traverser();
        $traverser->traverse($node, function (\EasyCI20220418\Nette\Neon\Node $node) use(&$classNames) {
            ...
            if (!$node->value instanceof \EasyCI20220418\Nette\Neon\Node\ArrayNode) {
                return null;
            }
            foreach ($node->value->items as $arrayItemNode) {
                ...
                if (!$arrayItemNode->value instanceof \EasyCI20220418\Nette\Neon\Node\LiteralNode) {
                    continue;
                }
