...
use EasyCI20220227\Nette\Neon\Encoder;
use EasyCI20220227\Nette\Neon\Neon;
...
final class ClassExtractor
{
     ...
     * @return string[]
     */
    public function extractFromFileInfo(\EasyCI20220227\Symplify\SmartFileSystem\SmartFileInfo $fileInfo) : array
    {
        $classNames = [];
        ...
        $classNameMatches = \EasyCI20220227\Nette\Utils\Strings::matchAll($fileContent, self::CLASS_NAME_REGEX);
        ...
        $staticCallsMatches = \EasyCI20220227\Nette\Utils\Strings::matchAll($fileContent, self::STATIC_CALL_CLASS_REGEX);
        foreach ($staticCallsMatches as $staticCallMatch) {
            $classNames[] = $this->extractClassName($fileInfo, $staticCallMatch);
        }
        return $classNames;
    }
    private function getFileContent(\EasyCI20220227\Symplify\SmartFileSystem\SmartFileInfo $fileInfo) : string
    {
        if ($fileInfo->getSuffix() === 'neon') {
            $neon = \EasyCI20220227\Nette\Neon\Neon::decode($fileInfo->getContents());
            // section with no classes that resemble classes
            unset($neon['application']['mapping']);
            unset($neon['mapping']);
            return \EasyCI20220227\Nette\Neon\Neon::encode($neon, \EasyCI20220227\Nette\Neon\Encoder::BLOCK, '    ');
        }
     ...
     */
    private function extractClassName(\EasyCI20220227\Symplify\SmartFileSystem\SmartFileInfo $fileInfo, array $match) : string
    {
