...
final class CheckFileClassNameCommand extends \EasyCI20220115\Symplify\PackageBuilder\Console\Command\AbstractSymplifyCommand
{
    /**
     * @var \Symplify\EasyCI\Psr4\RobotLoader\PhpClassLoader
     */
    private $phpClassLoader;
    public function __construct(\EasyCI20220115\Symplify\EasyCI\Psr4\RobotLoader\PhpClassLoader $phpClassLoader)
    {
    ...
    {
        /** @var string[] $sources */
        $sources = (array) $input->getArgument(\EasyCI20220115\Symplify\EasyCI\Psr4\ValueObject\Option::SOURCES);
        $classesToFiles = $this->phpClassLoader->load($sources);
        $missMatchingClassNamesByFiles = [];
        foreach ($classesToFiles as $class => $file) {
            $fileBasename = \pathinfo($file, \PATHINFO_FILENAME);
            $shortClassName = \EasyCI20220115\Nette\Utils\Strings::after($class, '\\', -1);
        ...
        foreach ($missMatchingClassNamesByFiles as $file => $class) {
            $fileInfo = new \EasyCI20220115\Symplify\SmartFileSystem\SmartFileInfo($file);
