...
final class SymplifyStrings
{
    ...
    {
        return \EasyCI20220530\Nette\Utils\Strings::substring($mainString, 0, -\strlen($stringToSubtract));
    }
    public function subtractFromLeft(string $mainString, string $stringToSubtract) : string
    {
        return \EasyCI20220530\Nette\Utils\Strings::substring($mainString, \strlen($stringToSubtract));
    }
    private function normalizePath(string $firstString) : string
    {
        return \EasyCI20220530\Nette\Utils\Strings::replace($firstString, '#\\\\#', '/');
    }
