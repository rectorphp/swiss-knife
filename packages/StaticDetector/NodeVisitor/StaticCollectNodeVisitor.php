...
final class StaticCollectNodeVisitor extends \EasyCI20220116\PhpParser\NodeVisitorAbstract
{
    ...
    public function enterNode(\EasyCI20220116\PhpParser\Node $node)
    {
        ...
        if ($node instanceof \EasyCI20220116\PhpParser\Node\Stmt\ClassMethod) {
            $this->enterClassMethod($node);
        }
        return null;
    }
    private function ensureClassLikeOrStaticCall(\EasyCI20220116\PhpParser\Node $node) : void
    {
        if ($node instanceof \EasyCI20220116\PhpParser\Node\Stmt\ClassLike) {
            $this->currentClassLike = $node;
        }
        if ($node instanceof \EasyCI20220116\PhpParser\Node\Expr\StaticCall) {
    ...
    private function enterClassMethod(\EasyCI20220116\PhpParser\Node\Stmt\ClassMethod $classMethod) : void
    {
        ...
        if ($this->currentClassLike === null) {
            ...
            throw new \EasyCI20220116\Symplify\SymplifyKernel\Exception\ShouldNotHappenException($errorMessage);
        }
