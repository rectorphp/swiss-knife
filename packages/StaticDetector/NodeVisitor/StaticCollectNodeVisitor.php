...
final class StaticCollectNodeVisitor extends \EasyCI20220325\PhpParser\NodeVisitorAbstract
{
    /**
     * @var string[]
     */
    ...
    public function __construct(\Symplify\EasyCI\StaticDetector\Collector\StaticNodeCollector $staticNodeCollector, \EasyCI20220325\Symplify\Astral\Naming\SimpleNameResolver $simpleNameResolver)
    {
    ...
    public function enterNode(\EasyCI20220325\PhpParser\Node $node)
    {
        ...
        if ($node instanceof \EasyCI20220325\PhpParser\Node\Stmt\ClassMethod) {
            $this->enterClassMethod($node);
        }
        return null;
    }
    private function ensureClassLikeOrStaticCall(\EasyCI20220325\PhpParser\Node $node) : void
    {
        if ($node instanceof \EasyCI20220325\PhpParser\Node\Stmt\ClassLike) {
            $this->currentClassLike = $node;
        }
        if ($node instanceof \EasyCI20220325\PhpParser\Node\Expr\StaticCall) {
    ...
    private function enterClassMethod(\EasyCI20220325\PhpParser\Node\Stmt\ClassMethod $classMethod) : void
    {
        ...
        if ($this->currentClassLike === null) {
            ...
            throw new \EasyCI20220325\Symplify\SymplifyKernel\Exception\ShouldNotHappenException($errorMessage);
        }
