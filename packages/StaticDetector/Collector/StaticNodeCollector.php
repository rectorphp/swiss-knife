...
use EasyCI20220213\PhpParser\Node\Expr\StaticCall;
...
final class StaticNodeCollector
{
    ...
    public function __construct(\EasyCI20220213\Symplify\Astral\Naming\SimpleNameResolver $simpleNameResolver)
    {
        $this->simpleNameResolver = $simpleNameResolver;
    }
    public function addStaticClassMethod(\EasyCI20220213\PhpParser\Node\Stmt\ClassMethod $classMethod, \EasyCI20220213\PhpParser\Node\Stmt\ClassLike $classLike) : void
    {
    ...
    public function addStaticCall(\EasyCI20220213\PhpParser\Node\Expr\StaticCall $staticCall) : void
    {
        if ($staticCall->class instanceof \EasyCI20220213\PhpParser\Node\Expr) {
            // weird expression, skip
            return;
        }
        if ($staticCall->name instanceof \EasyCI20220213\PhpParser\Node\Expr) {
            // weird expression, skip
            return;
        }
    ...
    public function addStaticCallInsideClass(\EasyCI20220213\PhpParser\Node\Expr\StaticCall $staticCall, \EasyCI20220213\PhpParser\Node\Stmt\ClassLike $classLike) : void
    {
        if ($staticCall->class instanceof \EasyCI20220213\PhpParser\Node\Expr) {
            // weird expression, skip
            return;
    ...
    private function resolveClass(\EasyCI20220213\PhpParser\Node\Name $staticClassName, \EasyCI20220213\PhpParser\Node\Stmt\ClassLike $classLike) : string
    {
        ...
        if ($class === 'parent') {
            if (!$classLike instanceof \EasyCI20220213\PhpParser\Node\Stmt\Class_) {
