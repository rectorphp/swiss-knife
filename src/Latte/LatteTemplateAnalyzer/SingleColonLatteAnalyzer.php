...
final class SingleColonLatteAnalyzer implements \Symplify\EasyCI\Latte\Contract\LatteTemplateAnalyzerInterface
{
    ...
    public function analyze(array $fileInfos) : array
    {
        ...
        foreach ($fileInfos as $fileInfo) {
            $matches = \EasyCI20220604\Nette\Utils\Strings::matchAll($fileInfo->getContents(), self::CLASS_CONSTANT_REGEX);
            if ($matches === []) {
                continue;
            }
        ...
        foreach ($fileInfos as $fileInfo) {
            $matches = \EasyCI20220604\Nette\Utils\Strings::matchAll($fileInfo->getContents(), self::CALL_REGEX);
            if ($matches === []) {
                continue;
            }
