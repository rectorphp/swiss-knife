...
use EasyCI20220417\Nette\Utils\DateTime;
use EasyCI20220417\Symplify\ComposerJsonManipulator\ComposerJsonFactory;
...
final class SupportedPhpVersionResolver
{
    ...
    public function __construct(\EasyCI20220417\Composer\Semver\VersionParser $versionParser, \EasyCI20220417\Composer\Semver\Semver $semver, \EasyCI20220417\Symplify\ComposerJsonManipulator\ComposerJsonFactory $composerJsonFactory)
    {
    ...
    public function resolveFromComposerJsonFilePath(string $composerJsonFilePath) : array
    {
        ...
        return $this->resolveFromConstraints($requirePhpVersion, \EasyCI20220417\Nette\Utils\DateTime::from('now'));
    }
    /**
     * @return string[]
     */
    public function resolveFromConstraints(string $phpVersionConstraints, \DateTimeInterface $todayDateTime) : array
    {
        ...
        foreach (\Symplify\EasyCI\ValueObject\PhpVersionList::VERSIONS_BY_RELEASE_DATE as $releaseDate => $phpVersion) {
            ...
            // is in the future?
            $relaseDateTime = \EasyCI20220417\Nette\Utils\DateTime::from($releaseDate);
