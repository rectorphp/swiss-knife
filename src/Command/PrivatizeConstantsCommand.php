...
final class PrivatizeConstantsCommand extends Command
{
    ...
    {
        if (\strpos($errorMessage, 'Access to undefined constant') === \false) {
            return null;
        }
        $match = \SwissKnife202405\Nette\Utils\Strings::match($errorMessage, self::PROTECTED_CONSTANT_MESSAGE_REGEX);
        ...
        foreach ($phpstanResult['files'] as $filePath => $detail) {
            foreach ($detail['messages'] as $messageError) {
                ...
                if ($publicClassConstMatch instanceof ClassConstMatch) {
                    ...
                    $classFileContents = \str_replace('private const ' . $publicClassConstMatch->getConstantName(), 'public const ' . $publicClassConstMatch->getConstantName(), $classFileContents);
                }
                if ($protectedClassConstMatch instanceof ClassConstMatch) {
                    // replace "private const NAME" with "protected const NAME"
                    $classFileContents = \str_replace('private const ' . $protectedClassConstMatch->getConstantName(), 'protected const ' . $protectedClassConstMatch->getConstantName(), $classFileContents);
                }
