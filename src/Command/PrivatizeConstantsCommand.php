...
final class PrivatizeConstantsCommand extends Command
{
    ...
    public function __construct(SymfonyStyle $symfonyStyle, ClassConstantFetchFinder $classConstantFetchFinder, ClassConstFinder $classConstFinder, TwigTemplateConstantExtractor $twigTemplateConstantExtractor)
    {
    ...
    protected function execute(InputInterface $input, OutputInterface $output) : int
    {
        ...
        $this->symfonyStyle->newLine(2);
        $this->symfonyStyle->success(\sprintf('Totally %d constants were made public', $visibilityChangeStats->getPublicCount()));
    ...
    private function processFileInfo(SplFileInfo $phpFileInfo, array $classConstantFetches) : VisibilityChangeStats
    {
        ...
        foreach ($classConstants as $classConstant) {
            if ($this->isClassConstantUsedPublicly($classConstantFetches, $classConstant)) {
                $changedFileContents = Strings::replace($phpFileInfo->getContents(), '#(public |    |\\t)const\\s+' . $classConstant->getConstantName() . '#', 'public const ' . $classConstant->getConstantName());
