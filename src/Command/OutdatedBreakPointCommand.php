...
use SwissKnife202504\Nette\Utils\Json;
...
use SwissKnife202504\Symfony\Component\Process\Process;
final class OutdatedBreakPointCommand extends Command
{
    ...
    protected function configure() : void
    {
        ...
        $this->addOption('limit', null, InputOption::VALUE_REQUIRED, 'Maximum number of outdated major version packages', 10);
    }
    protected function execute(InputInterface $input, OutputInterface $output) : int
    {
        ...
        foreach ($outdatedPackages as $outdatedPackage) {
            ...
            $symfonyStyle->writeln(\sprintf(' * Your version %s is <fg=%s>%s</>', $outdatedPackage->installedVersion, $tooOld ? 'red' : 'yellow', $outdatedPackage->installedAge));
        ...
        if ($outdatedPackageCount >= $maxOutdatePackages) {
            // to much → fail
            $symfonyStyle->error(\sprintf('There %s %d outdated package%s. Update couple of them to get under %d limit', $outdatedPackageCount > 1 ? 'are' : 'is', $outdatedPackageCount, $outdatedPackageCount > 1 ? 's' : '', $maxOutdatePackages));
            return self::FAILURE;
        }
        if ($outdatedPackageCount > \max(1, $maxOutdatePackages - 5)) {
            // to much → fail
            $symfonyStyle->warning(\sprintf('There are %d outdated packages. Soon, the count will go over %d limit and this job will fail.%sUpgrade in time', $outdatedPackageCount, $maxOutdatePackages, \PHP_EOL));
            return self::SUCCESS;
        }
        // to much → fail
    ...
    {
        $composerOutdatedProcess = Process::fromShellCommandline('composer outdated --no-dev --direct --major-only --format json');
    ...
    private function mapToOutdatedPackages(array $packagesData) : array
    {
        ...
        foreach ($packagesData as $packageData) {
            $outdatedPackages[] = new OutdatedPackage($packageData['name'], $packageData['latest'], $packageData['version'], $packageData['release-age']);
        }
