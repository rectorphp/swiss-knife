...
final class RenderMethodTemplateExtractor
{
    ...
    public function extractFromFileInfos(array $controllerFileInfos) : array
    {
        ...
        foreach ($controllerFileInfos as $controllerFileInfo) {
            $match = \EasyCI20220125\Nette\Utils\Strings::match($controllerFileInfo->getContents(), self::TEMPLATE_PATH_REGEX);
            if ($match === null) {
                continue;
            }
            /** @var string $relativeControllerFilePath */
            $relativeControllerFilePath = \EasyCI20220125\Nette\Utils\Strings::after($controllerFileInfo->getRealPath(), \getcwd() . '/');
