...
final class PhpFilesFinderTest extends AbstractTestCase
{
    ...
    {
        $finder = \Rector\SwissKnife\Finder\PhpFilesFinder::find([__DIR__ . '/Fixture'], ['*Controller.php']);

        $this->assertSame(1, count($finder));

        $file = array_pop($finder);
    ...
    {
        $finder = \Rector\SwissKnife\Finder\PhpFilesFinder::find([__DIR__ . '/Fixture'], ['*Model.php']);

        $this->assertSame(2, count($finder));
